...
- **Hardening de search_path — 21 funções que ficaram de fora da auditoria de 17/07 (19/07/2026).** Ao confirmar o fix de Storage acima, o advisor de segurança do Supabase apontou 21 funções sem `search_path` fixado: toda a família `pode_editar_*` (RDO, FVS, Pendências, Compras, Almoxarifado, Efetivo, Definições, Contratos, Medições) e os triggers de cálculo/trava que dependem dela (`recalcular_valor_medicao`, `recalcular_valor_contrato`, `saldo_material`, `valida_movimento_estoque`, `restringir_vencedor_item`, `recalcular_status_pedido`, `proximo_codigo_material`, `restringir_status_contrato`, `restringir_status_medicao`, `calcular_valor_item_medicao`, `validar_saldo_medicao`, `proteger_valores_medicao`). Todas anteriores a 17/07 — não é código novo, foram esquecidas na varredura original de `20260717_hardening_funcoes_privilegiadas.sql`, que cobriu só RDO/FVS/numeração/produção. Nenhuma é `SECURITY DEFINER`, mas quase todas chamam `meu_papel()`/`meus_modulos()` (que são) por referência não qualificada — o mesmo risco de search_path hijacking já corrigido nas outras. Fechado com `20260719_hardening_pode_editar_e_triggers.sql`, aplicada em produção; advisor confirmado limpo desses 21 itens.
...
- **Fase 3a (Financeiro — livro de lançamentos): entregue em 21/07/2026, aguardando teste de campo com dados reais após revisões obrigatórias** — detalhes em `docs/fase3_financeiro.md`. Inclui tabela `lancamentos_financeiros`, RLS por módulo `financeiro`, ingestão automática de Medições de empreiteiros aprovadas, ingestão de Compras conferidas com NF por item, tela `/financeiro` com lista/filtros/alertas/lançamento avulso/edição antes de pagar/baixa e script de importação de histórico em dry-run. Correções pós-revisão: fornecedor vencedor de Compras corrigido via `cotacao_item_vencedora_id`, edição de lançamento `a_pagar`, textos/mojibake do Financeiro e policy `lf_select` para rastreabilidade de lançamentos inativados. Fora de escopo: Curva S financeira, previsto x realizado, anexos fiscais e leitura automática de NF. Evolução em análise: documentos fiscais/anexos — ver `docs/superpowers/specs/2026-07-22-financeiro-notas-fiscais-insumo.md` e proposta Claude `docs/superpowers/specs/2026-07-22-financeiro-documentos-fiscais-design.md` (ainda não aprovada; não implementar).
...
- **Fase 6 (Suprimentos — Almoxarifado): entregue em 11/07/2026, aguardando teste de campo com movimentos reais e aceite** — detalhes em `docs/fase6_almoxarifado.md`. Completa a Fase 6. Estoque de materiais/EPIs/escritório com saldo, entrada vinculada a pedido de compra (substitui o recebimento manual de Compras — só habilitada para pedidos aprovado/enviado/recebido_parcial, por isso um pedido em rascunho não aparece pra vínculo), saída avulsa e lançamento de requisição, blocos de requisição em PDF pré-numerados, ferramentas próprias com empréstimo/devolução/atraso e conferência tripla no pedido de compra. Inventário dos materiais atualizado em 17/07/2026: 148 códigos receberam o saldo-alvo da coluna SALDO da planilha de julho/2026 (total 11.164 unidades), por ajuste transacional que preserva o histórico. Evoluções de 21–22/07: aba **Aluguéis** para equipamentos locados, com locadora, modalidade, chegada, vencimento, alerta de 1 dia, edição antes da entrega, quantidade locada e devolução parcial por histórico imutável (`ferramenta_locacoes_devolucoes`), fechando automaticamente quando a soma devolvida atinge o total. Por ter tabela nova e triggers encadeados, a evolução de devolução parcial exige revisão pós-commit do Claude Code antes de teste de campo com locação real.
...
- **Fase 7 (Extras) — parcial:** Galeria de Fotos implementada (09/07/2026); Gestão de Efetivo entregue em 12/07/2026 (fix crítico de RLS em 13/07 — ver abaixo), com botão **Editar trabalhador** acrescentado em 13/07/2026; aguardando teste de campo com trabalhadores reais e aceite — detalhes em `docs/fase7_efetivo.md`. **Definições de Projeto** (`/definicoes`) entregue em 13/07/2026 — decisões pendentes do cliente/proprietário, cliente em modo leitura, admin/equipe com módulo `definicoes` cria/edita/resolve. **Planejamento lookahead/PPC** (`/planejamento`) entregue em 19/07/2026 e ajustado em uso real — restrições vinculadas ao Cronograma, compromisso semanal, fechamento com PPC, visão mensal/trimestral, Gantt semanal do planejado e calendário mensal imprimível em A4; detalhes em `docs/fase7_planejamento.md`. Extra ainda pendente: Alertas.
...
- **Medições (`/contratos/:id/medicoes`, `/medicoes`) — regime empreiteiros entregue em 14/07/2026, aguardando teste de campo com uma medição real e aceite** — detalhes em `docs/fase7_medicoes.md`. Lança quantidade executada por item de contrato ativo, acumula saldo frente à quantidade contratada, calcula valor bruto/retido/líquido, aprovação exclusiva do admin com trava de saldo no banco sem exceção e PDF com identidade RT. Em 21/07 passou a alimentar o Financeiro automaticamente quando aprovada. Em 22/07 ganhou cancelamento auditado de medição aprovada (`status = cancelada`) via RPC admin-only, com motivo obrigatório e inativação dos lançamentos financeiros vinculados na mesma transação; a medição JFC `875f3d53-51b6-4763-9bde-7b4186e0af9d` foi formalizada como cancelada para fechar a auditoria da correção manual. Cliente não vê o módulo.
- **Medições — regime Produção própria (16/07/2026), segundo regime dentro do mesmo módulo `/medicoes`, aguardando teste de campo e aceite** — detalhes em `docs/fase7_producao_propria.md`. Cadastro de salário por trabalhador com vigência histórica e divisor fixo 30; produção diária de alvenaria/reboco por parede/face, preço por m² congelado por lançamento, rateio igual entre N profissionais; dia salarial integral com motivo e bloqueio de conflito; medição `MP-001...` por profissional e período; aprovação exclusiva do admin, snapshots imutáveis, status paga e cancelamento auditável. Evoluções de 17–21/07: planta PDF clicável, cadastro de paredes por pavimento, saldo independente por sobrado/serviço/face, impressão da planta, cores por status da parede (sem lançamento/parcial/100%), botão 100%/total, lançamentos recentes filtrados por unidade+serviço e edição de valor/data para corrigir erro de digitação. Cliente não vê o módulo.
...
- **Próxima etapa:** seguir `docs/sequencia-trabalho-codex-claude.md`: fechar aceites formais com dados reais dos módulos já entregues; não liberar teste de campo em fluxos de risco ainda pendentes de revisão pós-commit do Claude Code (ex.: devolução parcial de aluguéis e cancelamento de medições, se a revisão ainda não tiver sido feita). Para Financeiro, a Fase 3a está entregue; a evolução de documentos fiscais/anexos está apenas em análise/spec proposta, sem plano aprovado e sem implementação. Extra ainda não construído: Alertas. Pedido do Rodrigo em 13/07/2026 de anexar contrato assinado (PDF/foto) foi adiado — ver `docs/superpowers/specs/2026-07-13-fase7-medicoes-design.md` §9.
...
*Versão 1.19 — 22/07/2026 — §0 atualizado com o estado consolidado de 19–22/07: Fase 3a Financeiro entregue (`docs/fase3_financeiro.md`), Planejamento lookahead/PPC entregue (`docs/fase7_planejamento.md`), evolução de Produção própria por planta/parede, cancelamento auditado de Medições aprovadas, Aluguéis no Almoxarifado com equipamento/quantidade/devolução parcial e proposta ainda não aprovada de documentos fiscais/anexos financeiros (`docs/superpowers/specs/2026-07-22-financeiro-documentos-fiscais-design.md`).*
