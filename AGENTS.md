...
- **Auditoria geral de segurança e UX (iniciada 17/07/2026, em andamento)** — `docs/auditoria-geral-2026-07-17.md`. Corrigidos: leitura anônima de 11 tabelas (obras/unidades/etapas/cronograma/RDO — policies de SELECT sem `TO authenticated` caíam em `PUBLIC`, que inclui `anon`); 22 funções `SECURITY DEFINER` executáveis por `anon` por herança de `PUBLIC` (16 sem `search_path` fixo); cabeçalhos de segurança ausentes no frontend (CSP, nosniff, DENY iframe, referrer/permissions policy); tela branca do PWA após deploy (service worker servindo bundle antigo removido). Abertos: uploads sem limite de tamanho/MIME no backend; campos estourando painel no celular em Compras/Almoxarifado; tabelas largas (Compra/Contrato/Medição) dependendo de rolagem horizontal no mobile; `window.confirm/prompt` nativos fora da identidade visual; bundle principal ~1,16 MB sem code-splitting por rota.
- **Próxima etapa:** aceite formal da Fase 6 completa (Compras + Almoxarifado), da Gestão de Efetivo/Definições de Projeto (Fase 7 parcial) e agora também de Contratos e Medições (ambos os regimes — empreiteiros e produção própria), todos com uso real em andamento ou aguardando teste de campo; depois seguir para Financeiro (Fase 3, decidido por último pelo Rodrigo — não é predecessora de nada) ou fechar os achados abertos da auditoria de segurança/UX. Módulos aprovados mas ainda não construídos: Projetos (repositório de documentos versionado), Planejamento lookahead/PPC, Tarefas avulsas, Alertas. Pedido do Rodrigo em 13/07/2026 de anexar contrato assinado (PDF/foto) foi adiado — ver `docs/superpowers/specs/2026-07-13-fase7-medicoes-design.md` §9 e a conversa que o originou.
