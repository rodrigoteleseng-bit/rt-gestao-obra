...
- **Próxima etapa:** aceite formal da Fase 6 completa (Compras + Almoxarifado) e da Gestão de Efetivo/Definições de Projeto (Fase 7 parcial) com uso real em andamento; depois seguir para Medições (já tem o modelo de dois regimes definido — produção individual própria + empreiteiros por serviço, ver memória) ou Financeiro (Fase 3, decidido por último pelo Rodrigo — não é predecessora de nada). Módulos aprovados mas ainda não construídos: Projetos (repositório de documentos versionado), Planejamento lookahead/PPC, Tarefas avulsas.
