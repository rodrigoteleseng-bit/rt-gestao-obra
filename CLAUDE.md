...
- **Fase 7 (Extras) — parcial:** Galeria de Fotos implementada (09/07/2026); Gestão de Efetivo entregue em 12/07/2026, aguardando teste de campo com trabalhadores reais e aceite — detalhes em `docs/fase7_efetivo.md`. Cadastro nominal de trabalhadores, chamada diária de presença, RDO em rascunho passa a ler a chamada automaticamente (RDO assinado nunca lê a chamada — congelado no que foi assinado), banner "X de Y presentes hoje" no Dashboard. Demais extras pendentes.
...
- **Próxima etapa:** aceite formal da Fase 6 completa (Compras + Almoxarifado) com uso real em andamento e da Gestão de Efetivo (Fase 7 parcial) com trabalhadores reais; depois seguir para Medições ou Financeiro (Fase 3, decidido por último pelo Rodrigo — não é predecessora de nada).
