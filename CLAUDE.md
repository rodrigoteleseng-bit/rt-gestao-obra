...
- **Fase 7 (Projetos) — implementado localmente em 18/07/2026, aguardando commit externo e revisão pós-commit obrigatória do Claude Code antes de qualquer teste com dados reais** — detalhes em `docs/fase7_projetos.md`. Repositório versionado de documentos da obra (`/projetos`) com categorias projeto executivo/memorial/administrativo, revisão atual + histórico, upload PDF no bucket privado `projetos`, signed URL para abertura, cliente em leitura e escrita para admin/equipe com módulo `projetos`. Migração `supabase/migrations/20260718_projetos.sql` criada mas não aplicada ao Supabase.
