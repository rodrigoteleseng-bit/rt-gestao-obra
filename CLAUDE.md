...
- **Fase 7 (Projetos) — entregue em 18/07/2026, migração aplicada em produção, aguardando teste de campo com documentos reais e aceite** — detalhes em `docs/fase7_projetos.md`. Repositório versionado de documentos da obra (`/projetos`) com categorias projeto executivo/memorial/administrativo, revisão atual + histórico, upload PDF no bucket privado `projetos`, signed URL para abertura, cliente em leitura e escrita para admin/equipe com módulo `projetos`. Revisão prévia e pós-commit do Claude Code concluídas (achado crítico de trigger `BEFORE INSERT` corrigido antes da implementação); isolamento por obra estendido também ao Storage via `isolamento_obra_storage`.
