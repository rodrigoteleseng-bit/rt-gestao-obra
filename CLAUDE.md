...
- **Medições (`/contratos/:id/medicoes`, `/medicoes`) — entregue em 14/07/2026, aguardando teste de campo com uma medição real e aceite** — detalhes em `docs/fase7_medicoes.md`. Lança quantidade executada por item de contrato ativo (aceita fração, ex.: 1,2), acumula saldo frente à quantidade contratada, calcula valor bruto/retido/líquido (primeiro uso real do campo retenção % do contrato), aprovação exclusiva do admin com trava de saldo no banco sem exceção. PDF com identidade RT. Implementado via subagent-driven-development (5 tasks, cada uma revisada antes de avançar); a revisão da Task 1 encontrou 3 lacunas próprias de RLS/trigger (reatribuição de item de medição aprovada, trava de saldo somando por linha em vez de por item, valores calculados graváveis direto) — todas fechadas no mesmo dia antes de construir a interface, em `supabase/migrations/20260713_fase7_medicoes_travas.sql`. Fora de escopo: regime de mão de obra direta (produção própria), lançamento financeiro real (Fase 3 não existe), anexo de comprovante assinado (adiado a pedido do Rodrigo), vínculo automático com Avanço Físico. Cliente não vê o módulo.
...
- **Próxima etapa:** aceite formal da Fase 6 completa (Compras + Almoxarifado), da Gestão de Efetivo/Definições de Projeto (Fase 7 parcial) e agora também de Contratos e Medições, todos com uso real em andamento ou aguardando teste de campo; depois seguir para o regime de mão de obra direta das Medições (produção individual própria, spec separada) ou Financeiro (Fase 3, decidido por último pelo Rodrigo — não é predecessora de nada). Módulos aprovados mas ainda não construídos: Projetos (repositório de documentos versionado), Planejamento lookahead/PPC, Tarefas avulsas. Pedido do Rodrigo em 13/07/2026 de anexar contrato assinado (PDF/foto) foi adiado — ver `docs/superpowers/specs/2026-07-13-fase7-medicoes-design.md` §9 e a conversa que o originou.
