...
Uma tela `/dados-obra` onde o admin vê todas as obras cadastradas, edita os dados de qualquer uma, cria obras novas, e escolhe qual é a obra ativa — completando um mecanismo (`selecionarObra` no `ObraContext`) que já existe no código mas nunca foi chamado por nenhuma tela.
...
- **Obra ativa:** a tela também permite marcar qual obra é a ativa, chamando `selecionarObra(id)` (já existe em `ObraContext.tsx`, grava em `localStorage`) — hoje esse mecanismo não tem nenhum gatilho de UI em lugar nenhum do app.
...
1. **Lista de obras** — todas cadastradas (`ativo = true`, qualquer `status`), ordenadas por nome. Cada linha mostra: nome, cidade/estado, badge de status, e um selo "Ativa" na que é a `obraAtiva` do momento. Botão "Selecionar" nas demais linhas (some na que já é ativa).
...
- **Trocar obra ativa:** botão "Selecionar" chama `selecionarObra(id)` do `ObraContext` — mecanismo existente, sem alteração.
- **Após criar/editar/selecionar:** a lista recarrega; se a obra editada é a `obraAtiva`, o resto do app (Dashboard, header) reflete os novos dados na próxima renderização porque consome `obraAtiva` do mesmo contexto.
...
- Nenhuma mudança em `ObraContext.tsx` além do já existente `selecionarObra` sendo finalmente chamado por uma UI.
