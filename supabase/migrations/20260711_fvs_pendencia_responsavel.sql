-- ============================================================
-- FVS — responsável nas pendências automáticas | RT Engenharia
-- ============================================================
-- Pedido do Rodrigo em 11/07/2026: ao concluir uma verificação com
-- itens NC, poder indicar quem é o responsável pela correção — hoje
-- as pendências automáticas nasciam sem responsável.
-- Um responsável por rodada (vale para todas as pendências geradas);
-- pode ser ajustado depois na tela da pendência.

-- Mudança de assinatura da função (9 args) — sem o DROP explícito,
-- CREATE OR REPLACE criaria sobrecarga ambígua com a versão de 8 args.
DROP FUNCTION IF EXISTS concluir_verificacao_fvs(uuid, status_fvs, text, text, text, numeric, numeric, numeric);
...
BEGIN
  ...
  LOOP
    INSERT INTO pendencias (obra_id, unidade_id, tarefa_id, descricao, responsavel, fvs_id, criado_por)
    VALUES (
